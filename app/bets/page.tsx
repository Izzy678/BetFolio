...
export default async function BetsPage() {
  ...
  if (isSupabaseConfigured()) {
    ...
    const { data } = await supabase.from("bet_list").select("*").order("settled_at", { ascending: false }).range(0, 49);
    if (data) bets = data.map((row) => ({ id: row.id as string, bookmaker: row.bookmaker as string, externalBetId: row.external_bet_id as string | null, betType: row.bet_type as string, status: row.status as string, currency: row.currency as string, cashStake: Number(row.cash_stake), odds: row.total_odds_decimal == null ? null : Number(row.total_odds_decimal), pnl: Number(row.pnl), settledAt: row.settled_at as string | null })) satisfies BetListItem[];
  }
