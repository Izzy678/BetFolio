...
import { BarChart3, FileClock, Plus, Settings, X } from "lucide-react";
...
export function AppShell({ username, children }: { username: string; children: React.ReactNode }) {
  ...
  const nav = <>
    ...
    <div className="mt-auto border-t border-white/[.07] p-4"><div className="flex items-center gap-3 rounded-xl bg-white/[.035] p-3"><div className="grid size-9 place-items-center rounded-full bg-indigo-400/15 text-sm font-bold text-indigo-300">{username.slice(0, 1).toUpperCase()}</div><div className="min-w-0"><p className="truncate text-sm font-semibold text-zinc-200">@{username}</p><p className="text-xs text-zinc-600">Device account</p></div></div></div>
  </>;
