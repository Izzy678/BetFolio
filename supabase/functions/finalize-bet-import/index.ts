...
const statuses = new Set(["won","lost","void","push","cashout","pending"]);
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const bet = { ...body.bet, betType: body.bet.betType ?? "single", promotionalStake: 0, legs: [] };
    const amountFields = [bet.cashStake, bet.promotionalStake, bet.displayedReturn, bet.totalOddsDecimal].filter((value) => value != null);
    if (!statuses.has(String(bet.status)) || typeof bet.currency !== "string" || !/^[A-Z]{3}$/.test(bet.currency) || amountFields.some((value) => typeof value !== "number" || !Number.isFinite(value) || value < 0)) throw new Error("INVALID_IMPORT");
    if (bet.status === "won" && !["gross_return","net_profit"].includes(String(bet.returnKind))) bet.returnKind = "gross_return";
    if (bet.status === "cashout" && bet.returnKind !== "cashout") throw new Error("INVALID_IMPORT");
    if (bet.status === "won" && bet.displayedReturn == null) throw new Error("INVALID_IMPORT");
    if (bet.status === "cashout" && bet.displayedReturn == null) throw new Error("INVALID_IMPORT");
    ...
    if (error) {
      ...
      if (message.includes("BET_DUPLICATE") || error.code === "23505") throw new Error("BET_DUPLICATE");
      ...
      if (message.includes("INVALID") || message.includes("UNSUPPORTED") || message.includes("RETURN_REQUIRED")) throw new Error("INVALID_IMPORT");
      console.error("finalize_bet_import failed:", error);
